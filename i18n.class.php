...
class i18n {
    ...
    public function init( $outdated = false) {

        if (!$outdated && $this->isInitialized()) {
        ...
        if($this->filePath != null){
            ...
            $this->appliedLang = $this->userLangs[0];
        }
        ...
        if ($outdated) {
            ...
            if ($this->mergeFallback)
               ...
               // var_dump($this->appliedLang);die;
                $config = $this->createArrayFromShort($this->appliedLang);
                //var_dump($config);die;
              /*  var_dump($config);die;*/
    ...
    public function toStringDbParam(){

        return "Host: {$this->dbhost}, User : {$this->dbuser}, Password: {$this->dbpass}, Name: {$this->dbname}";
    }

    public function setAllDbParams($dbhost,$dbuser,$dbpass,$dbname){
    ...
    public function getDdConnection(){

        try {
            $this->odb = new PDO("mysql:host={$this->dbhost};dbname={$this->dbname}",  $this->dbuser, $this->dbpass);
    ...
    protected function createArrayFromShort($short){

        

        $value = $this->odb->prepare("SELECT i18n_category.I18CAT_name as cat, i18n.`I18N_term` as term, i18n_translate.i18TRA_term as translate  FROM `i18n` 
        INNER JOIN i18n_translate on i18n_translate.i18TRA_I18N_ID  = i18n.I18N_ID
        INNER JOIN i18n_language on i18n_language.I18LAN_ID = i18n_translate.i18TRA_I18LAN_ID
        INNER JOIN i18n_category on i18n_category.I18CAT_ID = i18n.I18N_I18CAT_ID
        WHERE i18n_language.I18LAN_short = :short ORDER BY i18n.I18N_I18CAT_ID DESC");

        $value->execute([":short"=>$short]);
      ...
      //  var_dump($value);die;
        ...
        foreach($value as $v){
            ...
            if($cat != $v["cat"]){
                ...
                $tmp[] = null;
               // $catArray[$cat] = $concat[sizeof($concat)-1];
                $catArray[$cat] = $current;
                $new = true;
                $concat = [];
                $current = [];
                $cat = $v["cat"];
